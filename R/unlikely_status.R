...
GROUP BY user, status >= %1$i",
  ...
  merge(
    status_obs[status_obs$above == 1, c("user", "n")],
    status_obs[status_obs$above == 0, c("user", "n")],
    by = "user", all = TRUE
  ) -> status_obs
  ...
  #users with the same pvalue in a group
  c(0, diff(status_obs$n.x) != 0 | diff(status_obs$n.y) != 0) |>
    cumsum() -> status_obs$group
  #sum log_p values within each group
  ...
  unlikely$reason <- sprintf(
    "fraction status above or equal to %i greater than %.0f%% for user %d",
    status_split, 100 * threshold, unlikely$user
  )
